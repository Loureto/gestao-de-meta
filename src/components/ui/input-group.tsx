...
const InputContainer = forwardRef<HTMLInputElement, InputGroupProps>(
...
const InputIcon = forwardRef<HTMLDivElement, InputIconProps>(
  ({ children, className, direction = 'left', ...props }, ref) => {
    const baseStyles = {
    ...
    return (
      <div
        ref={ref}
        className={cn(baseStyles.default, baseStyles[direction], className)}
        {...props}
      >
        {children}
      </div>
